...
function getSortedElephantsByNumber($elephants){
    ...
    //NOTES 2:You CAN'T use any sorting PHP built-in function.
     ...
     //Creamos un bucle que ira recorriendo el array
    for ($i = 0 ; $i <count($elephants);$i++){
        //creamos el segundo array para que no haya ningun fallo, es decir con un solo array
        // se puede quedar un numero sin girar asi que con el segundo nos cercionamos de que se coloca

        for ($j = 0; $j <count($elephants);$j++){

            //creamos una condicion que es "si" elefantes i es menor que elefantes J
            // cambias el valor de i de j es decir creamos una variable auxiliar para poder cambiar los datos.
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark fixed-top">
    <div class="container-fluid">
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <form class="d-flex" action="repasoejerciciosExamen.php">
        ...
        <?php
            ...
            //Aqui creamos un for que recorra la array para que vaya creando uno a uno
                for ($i = 0; $i < count($elephants);$i++) {
                    ...
                    echo "<br >";
                    echo "<h6 class='card-subtitle mb-2 text-muted' > Species: Asian </h6 ><br >";
                    echo "<h6 class='card-subtitle mb-2 text-muted' > Year of birth: ".$elephants[$i]['dob']." </h6 ><br >";
                    echo "<p class='card-text' > After attacking and nearly killing his keeper, Ziggy was chained to the wall of an indoor enclosure, and remained there for nearly three decades . In the late 1960s, when school children and other animal enthusiasts began campaigning for his release, eventually winning the case.</p ><br > ";
                    echo "<a class='btn mr-2' href = 'https://en.wikipedia.org/wiki/Ziggy_(elephant)' target = '_blank' >";
                    echo "<i class='fas fa-link' ></i > Visit elephant </a >";
                    echo "</div ></div ></div >";
}
        ?>
